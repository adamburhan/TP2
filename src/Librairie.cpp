...
namespace
{
    constexpr std::size_t FILM_INEXSISTANT = (std::size_t)-1;
} // namespace
...
{
    std::size_t indexFilm = trouverIndexFilm(nomFilm);
...
std::ostream& operator<<(std::ostream& outputStream, const Librairie& librairie)
{
    for (std::size_t i = 0; i < librairie.films_.size(); i++)
    {
        outputStream << *librairie.films_[i];
        outputStream << '\n';
    }
    return outputStream;
}
...
std::size_t Librairie::trouverIndexFilm(const std::string& nomFilm) const
{
